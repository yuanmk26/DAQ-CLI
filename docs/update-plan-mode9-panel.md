...
│ B. 过阈链路 (TCM)                                                 │
│   0x45~64 阈值（2 列 × 8 行 grid，每通道独立）:                    │
│                                     ch00[____] ch08[____]         │
│                                     ch01[____] ch09[____] ...     │
│   广播填值 [____] [填到 16 通道]   ← 快捷方式，不影响逐通道       │
...
- **阈值逐通道独立**：0x45~0x64 是 16 个独立寄存器，面板每通道一个输入框
  （不同通道基线/增益不同，应各填各的——参考 measure 工具的每通道建议值）。
  「广播填值」只是填表快捷方式，不改变逐通道语义

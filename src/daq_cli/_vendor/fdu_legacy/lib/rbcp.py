...
class Rbcp(object):
    ...
    # def __init__(self, device_ip="192.168.10.12", udp_port=4660, timeout=SOCKET_TIMEOUT):
    #     """
    #     Constructor.
    #     :type device_ip: str
    ...
    def __init__(self, device_ip=None, udp_port=None, timeout=SOCKET_TIMEOUT):
        """
        Constructor.
        :type device_ip: str or None
        :param device_ip: SiTCP Device IP Address. If None, read from environment variable.
        :type udp_port: int or None
        :param udp_port: SiTCP Device RBCP(UDP) Port. If None, read from environment variable.
        :type timeout: int
        :param timeout: The number of milliseconds that the socket times out.
        """
        import os
        ...
        if device_ip is None:
            device_ip = os.environ.get('SITCP_DEVICE_IP', '192.168.10.11')

        # 如果未提供端口，从环境变量读取
        if udp_port is None:
            udp_port = int(os.environ.get('SITCP_UDP_PORT', '4660'))
